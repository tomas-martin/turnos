  ...
  static async getStats(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const paidPayments = await prisma.payment.findMany({
        where: {
          appointment: { branch: { companyId }, status: AppointmentStatus.COMPLETED },
      ...
      const cancellationsCount = await prisma.appointment.count({
        where: { branch: { companyId }, status: AppointmentStatus.CANCELLED }
      });

      const noShowsCount = await prisma.appointment.count({
        where: { branch: { companyId }, status: AppointmentStatus.NO_SHOW }
      });
      ...
      const hourBookings = await prisma.appointment.groupBy({
        by: ['startTime'],
        where: { branch: { companyId } },
        _count: { id: true }
      });
      ...
      const last7DaysData: { date: string; turnos: number }[] = [];
      for (let i = 6; i >= 0; i--) {
        const d = new Date();
