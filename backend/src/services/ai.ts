...
import { AIStatsService } from './aiStats';

export class AIService {
  private static getOllamaUrl() {
    return process.env.OLLAMA_URL || 'http://localhost:11434';
  }

  private static getOllamaModel() {
    return process.env.OLLAMA_MODEL || 'qwen2.5:1.5b';
  }

  static async queryAI(companyId: string, userQuestion: string): Promise<string> {
    try {
      // 1. Obtener las estadísticas actuales del negocio en tiempo real
      const stats = await AIStatsService.getBusinessSummary(companyId);
