...
export class AIStatsService {
  ...
  private static parseCleanDate(date: Date): Date {
  ...
  static async getBusinessSummary(companyId: string) {
    ...
    const employeeStats = await prisma.appointment.groupBy({
      by: ['employeeId'],
      where: {
        branch: { companyId },
    ...
    const tomorrowAvailability: { employeeName: string; slots: string[] }[] = [];
    ...
    const formatTomorrowSlots = tomorrowAvailability.map(ea => 
      `- **${ea.employeeName}**: ${ea.slots.length > 0 ? ea.slots.join(', ') : 'Sin horarios disponibles'}`
    ).join('\n');
