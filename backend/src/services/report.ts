...
export class ReportService {
  private static async getReportData(companyId: string, filters: {
    startDate?: string;
    endDate?: string;
    employeeId?: string;
  }) {
  ...
  static async generateCSV(companyId: string, filters: any): Promise<string> {
  ...
  static async generateExcel(companyId: string, filters: any): Promise<Buffer> {
    const appointments = await this.getReportData(companyId, filters);
  ...
  static async generatePDF(companyId: string, filters: any): Promise<Buffer> {
    const appointments = await this.getReportData(companyId, filters);
    ...
    return new Promise((resolve, reject) => {
      ...
      const completedTurns = appointments.filter(a => a.status === AppointmentStatus.COMPLETED).length;
      const cancelledTurns = appointments.filter(a => a.status === AppointmentStatus.CANCELLED).length;
      const totalRevenue = appointments
        .filter(a => a.status === AppointmentStatus.COMPLETED && a.payment?.status === PaymentStatus.PAID)
