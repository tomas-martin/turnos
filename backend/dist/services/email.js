...
class EmailService {
    static emailFrom = process.env.EMAIL_FROM || 'no-reply@turnos-saas.com';
    static getHTMLTemplate(title, bodyContent, actionButton) {
        return `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    static async sendAppointmentConfirmation(details) {
    ...
    static async sendAppointmentCancellation(details) {
        const dateStr = this.formatDate(details.appointment.date);
        const body = `
    ...
    static async sendAppointmentReschedule(details, oldDate, oldStartTime) {
        const dateStr = this.formatDate(details.appointment.date);
